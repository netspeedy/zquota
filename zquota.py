#!/usr/bin/env python3
"""zquota - Z.ai quota monitor with human-friendly, JSON, and raw output formats.

Copyright (c) 2026 Simon Oakes
MIT License - see https://github.com/netspeedy/zquota/blob/main/LICENSE
"""
...
def _run_loop(args: argparse.Namespace, use_color: bool) -> int:
    ...
    while True:
        if watch_mode:
            ...
            sys.stdout.write(
                color(
                    f"  zquota - refreshing every {args.watch}s  (Ctrl-C to stop)\n\n",
                    COLOR_DIM,
                    use_color,
                )
            )
