...
curl -fsSL ${metadata.siteUrl}zquota-archive-keyring.gpg \\
...
sudo apt update && sudo apt install zquota</code></pre>`;
  }
...
<pre><code>curl -fsSL https://github.com/netspeedy/zquota/releases/latest/download/zquota-${metadata.releaseNumber}.tar.gz -o zquota.tar.gz
tar -xzf zquota.tar.gz
sudo install -m 0755 zquota-${metadata.releaseNumber}/zquota.py /usr/local/bin/zquota</code></pre>`;
  }
    ...
    highlightsList.replaceChildren(
      ...metadata.releaseHighlights.map((highlight) => {
        ...
        if (match) {
          ...
          link.href = `https://github.com/netspeedy/zquota/commit/${match[2]}`;
